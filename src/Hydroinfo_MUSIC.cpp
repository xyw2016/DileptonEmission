...
void Hydroinfo_MUSIC::readHydroData(int whichHydro, int nskip_tau_in, int flag_tz) {
  ...
  if (use_tau_eta_coordinate == 0) {
    cout << "Hydroinfo_MUSIC:: Warning hydro grid is set to "
         << "cartesian coordinates, please make sure this is correct!" << endl;
  }
  ...
  if (whichHydro == 6) {
    ...
    while (true) {
      ...
      int ieta =
          (static_cast<int>(ik / (num_fluid_cell_trans)) - n_eta * itau_idx);
      ...
      if ((ik - 1) % (num_fluid_cell_trans * n_eta) == 0) {
        cout << "read in tau frame: " << itau_idx
             << " tau_local = " << setprecision(3) << tau_local << " fm ..."
             << endl;
      }
    ...
    cout << "Reading event-by-event hydro evolution data from JF ..." << endl;
    ...
    cout << "Using new MUSIC hydro format (no grid) reading data ..." << endl;
    if (whichHydro == 10) {
      boost_invariant_ = false;
    } else {
      boost_invariant_ = true;
    ...
    for (int i = 0; i < static_cast<int>(lattice_3D_ideal.size()); i++) {
      ...
      int cell_idx =
          (((cell_i.itau * ietamax + cell_i.ieta) * ixmax + cell_i.iy) * ixmax +
           cell_i.ix);
      idx_map_[cell_idx] = i;
    }
    ...
    while (true) {
      ...
      if (status != nVar_per_cell) {
        cerr << "[Hydroinfo_MUSIC::readHydroData]: ERROR: "
             << "the evolution file format is not correct"
             << " " << status << endl;
        exit(1);
      }
      ...
      newCell.pressure = cell_info[5];
      newCell.temperature = cell_info[6];
      if (newCell.temperature > hydroTmax)
        hydroTmax = newCell.temperature;
      if (newCell.temperature < hydroTmin)
        hydroTmin = newCell.temperature;
      ...
      if (turn_on_bulk == 1) {
        newCell.bulkPi = (cell_info[11 + turn_on_rhob * 2 + turn_on_shear * 5]);
      } else {
        newCell.bulkPi = 0.;
      }
      if (turn_on_diff == 1) {
        newCell.qx = (cell_info[11 + turn_on_rhob * 2 + turn_on_shear * 5 +
                                turn_on_bulk]);
        newCell.qy = (cell_info[12 + turn_on_rhob * 2 + turn_on_shear * 5 +
                                turn_on_bulk]);
        newCell.qz = (cell_info[13 + turn_on_rhob * 2 + turn_on_shear * 5 +
                                turn_on_bulk]);
      } else {
        newCell.qx = 0.;
        newCell.qy = 0.;
      ...
      // std::cout << newCell.muB<< " " <<newCell.temperature << "
      // "<<newCell.ux<<" "<<newCell.uy<<" "<<newCell.ueta<<" "
      //          << newCell.pi11 << " "<<newCell.pi12 <<" "<<newCell.pi13 <<"
      //          "<<newCell.pi22 <<" "<<newCell.pi23  <<" "<< newCell.qx
      //          << newCell.qy << " "<<newCell.qz <<" "<<newCell.ed<<"
      //          "<<newCell.pressure <<std::endl;
  ...
  if (whichHydro == 6) {
    hydroTauMax =
        (hydroTau0 +
         hydroDtau * static_cast<int>(static_cast<float>(lattice_3D.size()) /
  ...
  if (whichHydro == 8 || whichHydro == 9) {
    hydroTauMax =
        (hydroTau0 +
         hydroDtau * static_cast<int>(static_cast<float>(lattice_2D.size()) /
                                          ((2. * hydroXmax / hydroDx) *
                                           (2. * hydroXmax / hydroDx)) -
                                      1));
...
void Hydroinfo_MUSIC::getHydroValues(float x, float y, float z, float t,
                                     fluidCell *info) {
  ...
  if (itau < 0 || itau > itaumax) {
    ...
    cout << "[Hydroinfo_MUSIC::getHydroValues]: tau= " << tau
         << ", hydroTauMax = " << hydroTauMax << ", hydroDtau = " << hydroDtau
         << endl;
    ...
    for (int ipeta = 0; ipeta < 2; ipeta++) {
      ...
      for (int ipy = 0; ipy < 2; ipy++) {
        ...
        if (hydroWhichHydro == 8 || hydroWhichHydro == 9) {
          HydroCell_2D_ptr1 = (&lattice_2D[position[0][ipy][ipeta][iptau]]);
          HydroCell_2D_ptr2 = (&lattice_2D[position[1][ipy][ipeta][iptau]]);
          HydroCell2DInterp =
              HydroCell2DInterp + ((*HydroCell_2D_ptr1) * (1. - xfrac) +
                                   (*HydroCell_2D_ptr2) * xfrac) *
                                      prefrac;
        } else if (hydroWhichHydro == 6) {
          HydroCell_3D_ptr1 = (&lattice_3D[position[0][ipy][ipeta][iptau]]);
          HydroCell_3D_ptr2 = (&lattice_3D[position[1][ipy][ipeta][iptau]]);
          HydroCell3DInterp =
              HydroCell3DInterp + ((*HydroCell_3D_ptr1) * (1. - xfrac) +
          ...
          HydroCell3DnewInterp =
              (HydroCell3DnewInterp + ((*HydroCell_3D_new_ptr1) * (1. - xfrac) +
  ...
  for (int i = 0; i < itaumax; i++) {
    ...
    for (int ix = 0; ix < ixmax; ix++) {
      ...
      for (int iy = 0; iy < ixmax; iy++) {
        ...
        temp_evo << scientific << setw(16) << setprecision(8) << temp_local
                 << "   ";
      }
      temp_evo << endl;
    }
...
void Hydroinfo_MUSIC::update_grid_info(float tau0, float tau_max, float dtau,
                                       float x_max, float dx, float eta_max,
                                       float deta) {
  hydroTau0 = tau0;
  hydroTauMax = tau_max;
  hydroDtau = dtau;
