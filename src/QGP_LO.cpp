...
QGP_LO::QGP_LO(std::shared_ptr<ParameterReader> paraRdr_in,
...
double QGP_LO::cross_sec(double omega, double q, double qsq, double m_ell2) {
  ...
  return 4 * Nc * C_EM * (4. * M_PI / 3.) * (pow(alphaEM, 2) / qsq) *
         Bfun(m_ell2 /
              qsq); // 4=initial spin configurations not to be average. i.e.,
                    // the x-sec used used be 4-times larger than p&s (5.13)
}

// for a_1 part of diff rate
double QGP_LO::integrand_J(double x, int n, double a,
                           double z) { // define the integrand function
  return pow(x, n) /
         ((exp(x - z) + 1.) * (exp(a - x + z) + 1.) * (exp(z - x) + 1.));
}

double QGP_LO::gaussLegendre(double *xs, double *ws, int m, int n, double a,
                             double b, double z) {
  double integral = 0.0;
  double c = a / 2.0;
  double d = b / 2.0;
...
double QGP_LO::intJn(
    ...
    double z) {     // computing J_n(a,b,z) in eq.19; a=q^0/T, b=|q|/T, z=muB/3T
  const int m = 31; // m=31 points gauss integral
  double xs[m] = {
      9.97087482e-01,  9.84685910e-01,  9.62503925e-01,  9.30756998e-01,
...
double QGP_LO::a1(double omega, double q, double qsq, double T, double muB,
                  double m_ell2, double sigma, double nB_o_epp) {
  // omega = q^0, q = magnitude of 3-vec q, T = temperature, muB = Baryon
  // chemical potential, m_ell2 = the lepton mass squared nB_o_epp = n_B/(e+p)
  // must be import from hydro final rate for a1 is d4R/d4q =
  // a1*q.V/(T^2*kappa-hat) where V is the diffusion current note that the
  // factor T*qsq/(4.*pow((2.*M_PI),5)*pow(q,3)) is not included in a1 here.
  // Moved to PhotonEmission.cpp

  double muq = muB / 3.;
  // double sigma = cross_sec(omega,q,qsq,m_ell2);
  // double ps1 = T*qsq*sigma/(4.*pow((2.*M_PI),5)*pow(q,3));
  double ps2 =
      2 * omega * nB_o_epp * pow(T, 3) * intJn(1, omega / T, q / T, muq / T) -
      pow(T, 2) * (qsq * nB_o_epp + 2. * omega / 3.) *
          intJn(0, omega / T, q / T, muq / T) +
...
double QGP_LO::s2(double omega, double q, double qsq, double T, double muB,
                  double m_ell2, double sigma) {
  // omega = q^0, q = magnitude of 3-vec q, qsq = (q^0)^2 - q^2, T =
  // temperature, muB = Baryon chemical potential, m_ell2 = the lepton mass
  // squared The final DPR is given by deltaDPR_shear = q^a q^b pi^ab *s2
  // /(T^2(E+P)) note that the factor Cq*T*qsq/(4.*pow((2.*M_PI),5)*pow(q,5)) is
  // not included in s2 here. Moved to PhotonEmission.cpp

  double muq = muB / 3.;

  double j2 = intJn(2, omega / T, q / T, -muq / T) +
              intJn(2, omega / T, q / T, muq / T);
  double j1 = intJn(1, omega / T, q / T, -muq / T) +
              intJn(1, omega / T, q / T, muq / T);
  double j0 = intJn(0, omega / T, q / T, -muq / T) +
              intJn(0, omega / T, q / T, muq / T);

  // double sigma = cross_sec(omega,q,qsq,m_ell2);
  double ps2 = (3. * pow(omega, 2) - pow(q, 2)) * pow(T, 2) * j2 -
               3. * omega * qsq * T * j1 + 3. * pow(qsq, 2) * j0 / 4.;
...
double QGP_LO::nB(double x) { return 1. / (exp(x) - 1.); }
...
double QGP_LO::rhoV(double omega, double k, double ksq, double T, double muB) {
  ...
  double ps2 = l1f((kplus - muq) / T) - l1f((fabs(kminus) - muq) / T) +
               l1f((kplus + muq) / T) - l1f((fabs(kminus) + muq) / T);
...
double QGP_LO::rhoV2(double omega, double k, double ksq, double T, double muB) {
  ...
  double Nc = 3.;
  double ps1 = Nc * ksq / (4. * M_PI * k);
  // double ps2 = l1f((kplus - muq)/T) - l1f((abs(kminus) - muq)/T) + l1f((kplus
  // + muq)/T) - l1f((abs(kminus) + muq)/T); double ps3 = k*heaviside(kminus);
  double ps2 = log((cosh(kplus / T) + cosh(muq / T)) /
                   (cosh(kminus / T) + cosh(muq / T)));
  // return ps1*(T*ps2 + ps3);
  return ps1 * (T * ps2);
}
...
double QGP_LO::rhoL(double o, double k, double K2, double T, double muB) {
  ...
  double ps2 =
      l2f(kp + mu) + l2f(kp - mu) + somk * (l2f(km + mu) + l2f(km - mu));
...
void QGP_LO::fmuB_rate(double omega, double q, double qsq, double T, double muB,
                       double m_ell2, double &rV, double &rL, double &rT) {
  // omega = q^0, q = magnitude of 3-vec q, T = temperature, muB = Baryon
  // chemical potential, m_ell2 = the lepton mass squared

  double C_EM = 2. / 3.;
  double prefacor = C_EM * pow(alphaEM, 2) * nB(omega / T) *
                    Bfun(m_ell2 / qsq) / (3. * pow(M_PI, 3) * qsq);
...
void QGP_LO::FiniteBaryonRates(double T, double muB, double inv_eplusp,
                               double rhoB_over_eplusp, double Eq, double M_ll,
                               double &eqrate_ptr, double &eqrateT_ptr,
                               double &eqrateL_ptr, double &viscrate_ptr,
                               double &diffrate_ptr, int include_visc_deltaf,
