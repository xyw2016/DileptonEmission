...
void HydroinfoH5::addGroupattributeInt(hid_t H5groupEventid, string attName,
                                       int attValue) {
  hsize_t dims;
...
void HydroinfoH5::addGroupattributeDouble(hid_t H5groupEventid, string attName,
                                          double attValue) {
  hsize_t dims;
  hid_t attribute_id, dataspace_id;
  ...
  dataspace_id = H5Screate_simple(1, &dims, NULL);
  /* Create a dataset attribute. */
  attribute_id = H5Acreate(H5groupEventid, attName.c_str(), H5T_NATIVE_DOUBLE,
                           dataspace_id, H5P_DEFAULT, H5P_DEFAULT);
...
void HydroinfoH5::writeHydroBlock(
  ...
  groupFrameid = H5Gcreate(H5groupEventid, frameName.str().c_str(), H5P_DEFAULT,
...
void HydroinfoH5::readHydroinfoSingleframe(int frameIdx) {
  ...
  if (frameIdx < (int)grid_Framenum) {
    ...
    cout << "Error: readHydroinfoSingleframe :: frameIdx exceed maximum frame "
            "number from hydro"
         << endl;
...
void HydroinfoH5::getHydroinfoOnlattice(int frameIdx, int xIdx, int yIdx,
  ...
  if (frameIdx < 0 || frameIdx > grid_Framenum || xIdx < 0 ||
      xIdx > (grid_XH - grid_XL) || yIdx < 0 || yIdx > (grid_YH - grid_YL)) {
    cout << "Error: getHydroinfoOnlattice:: Index is wrong" << endl;
    cout << "frameIdx = " << frameIdx << " xIdx = " << xIdx << "yIdx = " << yIdx
         << endl;
    exit(1);
  }
...
double HydroinfoH5::cubeInterpShell(int idx_x, int idx_y, int idx_z, double x,
                                    double y, double z, double ***dataset) {
...
double HydroinfoH5::cubeInterp(double x, double y, double z, double A000,
                               double A100, double A010, double A110,
                               double A001, double A101, double A011,
                               double A111)
