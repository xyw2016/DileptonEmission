...
QGP_LO_analytic::QGP_LO_analytic(std::shared_ptr<ParameterReader> paraRdr_in,
                                 std::string emissionProcess)
    : ThermalPhoton{paraRdr_in, emissionProcess} {}

// PRC. 93, 044902, 2016
void QGP_LO_analytic::FiniteBaryonRates(
  ...
  double prefac = (Qu * Qu + Qd * Qd + Qs * Qs) * aem * aem /
                  (2. * pow(M_PI, 4)) / pow(hbarC, 4);
  // if (Eq < M_ll)
  ...
  // std::cout<<  eqrate_ptr<<" " <<prefac << " "<<y<<" "<<fq<<" "<<log_r<<
  // std::endl;
}
