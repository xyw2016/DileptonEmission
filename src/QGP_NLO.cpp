...
void error_handler_nlo(const char *reason, const char *file, int line,
                       int gsl_errno) {
  // fprintf(stderr, "GSL ERROR: %s:%d: %s (error code: %d)\n", file, line,
  // reason, gsl_errno);
}
QGP_NLO::QGP_NLO(std::shared_ptr<ParameterReader> paraRdr_in,
...
double rhoV_AMY(double alpha, double mu,
                double k) { // (1.9) and (1.10) of hep-ph/0111107
  double nf = 3.;
  double minf2, fcol, f2t2;
  minf2 = 4. * M_PI * alpha * (Cf / 4.) * (1. + 1. * mu * mu / (M_PI * M_PI));
  f2t2 = .041 / k - .3615 + 1.01 * exp(-1.35 * k);
  fcol = sqrt(1. + nf / 6.) * (.548 * log(12.28 + 1. / k) * pow(k, -1.5) +
...
void rho_OPE(double o, double k, double alpha, double mu, double &rT,
             double &rL) {
...
ThermalPhoton::Table::Table(std::vector<double> &_x, std::vector<double> &_y,
                            std::vector<double> &_z, std::vector<double> &_w,
                            std::vector<double> &_F)
    : nx(_x.size()), ny(_y.size()), nz(_z.size()), nw(_w.size()), x(&_x[0]),
      y(&_y[0]), z(&_z[0]), w(&_w[0]) {
...
double ThermalPhoton::Table::interp(double _x, double _y, double _z,
                                    double _w) {
  // Implementation of Table member function interp...

  int i = id(_x, x, nx), j = id(_y, y, ny), k = id(_z, z, nz),
      l = id(_w, w, nw);
  double res, a, b, c, d;
  a = (_x - x[i]) / (x[i + 1] - x[i]);
  b = (_y - y[j]) / (y[j + 1] - y[j]);
  ...
  res = (1. - a) * (1. - b) * (1. - c) * (1. - d) * F[i][j][k][l] +
        ...
        (a) * (b) * (c) * (d)*F[i + 1][j + 1][k + 1][l + 1]; // hypercube
  return res;
}

void ThermalPhoton::initialize(string fname, std::vector<double> &a_list,
                               std::vector<double> &B_list,
                               std::vector<double> &M_list,
                               std::vector<double> &k_list,
                               std::vector<double> &rhoT_list,
  ...
  while (true) {
    ...
    if (a_list.back() < alpha)
      a_list.push_back(alpha);
    if (B_list.back() < muB)
      B_list.push_back(muB);
    if (M_list.back() < M)
      M_list.push_back(M);
    if (k_list.back() < k)
      k_list.push_back(k);
...
int approx_rho(void *input, // omega, k, alpha, muB
               ...
               struct ThermalPhoton::Table rhoL_list, double &rT,
               double &rL // out: T & L spectral functions
) {
  ...
  if (alpha <
      al_min - 1e-5) { // use LO result to extrapolate if alpha < alpha_min
    // cout << "using LO...\n";
    double LO_rT, LO_rL;
  ...
  if (M > M_max + 1e-5) { // use OPE expansion to extrapolate if M > M_max
    ...
    // cout << "using AMY...\n";
    double _in[4] = {sqrt(M_min * M_min + k * k), k, alpha,
                     mu}; // use boundary of grid
    double min_rT, min_rL;
    approx_rho(_in, rhoT_list, rhoL_list, min_rT, min_rL);
  ...
  rL = rhoL_list.interp(alpha, muB, M, k);
  return 0;
}
...
void ThermalPhoton::NLO_rate(struct ThermalPhoton::Table grid_T,
                             struct ThermalPhoton::Table grid_L, double o,
                             double k, double alpha_s, double muB, double T,
                             double m_l, double &rateTot, double &rateT,
                             double &rateL) {
  // NB: quantities are defined in the local rest frame, i.e. u_\mu=(1,0,0,0)
  double M2 = o * o - k * k;

  double prefactor =
      B_(m_l * m_l / M2) * (2. / 3.) * pow(hbarC, -4.) * pow(alphaEM, 2.);
  // units: GeV^-4.fm^-4

  // m_l, T, muB, o and k must all be given in units of GeV!
  double in[4] = {o / T, k / T, alpha_s,
                  muB / T}; // spf is a function of o/T etc.
  double rhoT_app, rhoL_app;
  approx_rho(in, grid_T, grid_L, rhoT_app, rhoL_app);

  double prefac =
      prefactor * nB(o / T) * pow(T, 2.) / (3. * pow(M_PI, 3.) * M2);
