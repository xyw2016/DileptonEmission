...
std::vector<double> logarithmic_mass_grid(double m_min, double m_max,
                                          int num_points) {
...
// Note also that the last line of data needs a return in order to be read.
{
  ...
  if (lineSize == 0) {
    // empty:
    cout << "readBlockData warning: input stream has empty first row; no data "
            "read"
         << endl;
    return NULL;
  } else {
...
// -- x0: where the interpolation should be performed
{
  ...
  if (idx == 0) {
    ...
    double A0 = (*y)[0], A1 = (*y)[1], A2 = (*y)[2],
           deltaX = x0 - (*x)[0]; // deltaX is the increment of x0 compared to
...
// -- xx: where the interpolation should be performed
{
  ...
  if (idx == 0) {
    ...
    return (*y)[size - 2] + ((*y)[size - 1] - (*y)[size - 2]) /
                                ((*x)[size - 1] - (*x)[size - 2]) *
                                (xx - (*x)[size - 2]);
  } else {
    // use cubic interpolation
    ...
    C0 = (x0 * x02 * x2 * x03 * x23 * x3 * y1 +
          x1 * x1s *
              (x0 * x03 * x3 * y2 + x2s * (-x3 * y0 + x0 * y3) +
               x2 * (x3s * y0 - x0s * y3)) +
          x1 * (x0s * x03 * x3s * y2 + x2 * x2s * (-x3s * y0 + x0s * y3) +
                x2s * (x3 * x3s * y0 - x0 * x0s * y3)) +
          x1s * (x0 * x3 * (-x0s + x3s) * y2 + x2 * x2s * (x3 * y0 - x0 * y3) +
    ...
    C1 = (x0s * x03 * x3s * y12 + x2 * x2s * (x3s * y01 + x0s * y13) +
         ...
         denominator;
    C2 =
        (-x0 * x3 * (x0s - x3s) * y12 + x2 * (x3 * x3s * y01 + x0 * x0s * y13) +
...
// -- xx: where the interpolation should be performed
{
  ...
  return (*y)[idx] + ((*y)[idx + 1] - (*y)[idx]) / ((*x)[idx + 1] - (*x)[idx]) *
                         (xx - (*x)[idx]);
}
...
//**********************************************************************
double invertFunc(double (*func)(double), double y, double xL, double xR,
                  double dx, double x0, double relative_accuracy)
// Purpose:
...
// Solve: f(x)=0 with f(x)=table(x)-y => f'(x)=table'(x)
{
  ...
  XX2 = x0;
  XX1 =
      XX2 - 10 * accuracy; // this value 10*accuracy is meanless, just to make
...
double invertTableDirect(vector<double> *x, vector<double> *y, double y0,
...
//  -- x0: initial guess
{
  ...
  zq_x_global = x;
  zq_y_global = y;
  return invertFunc(&invertTableDirect_hook, y0, (*x)[0], (*x)[size - 1],
                    (*x)[1] - (*x)[0], x0, relative_accuracy);
}
...
// using binary search.
{
  ...
  idx_f = length - 1;
  if (value > (*A)[idx_f]) {
    cout << "binarySearch: desired value is too large,  exceed the end of the "
            "table."
         << endl;
    exit(1);
  }
...
void outputFunctionerror(string function_name, string massage, double value,
                         int level) {
  switch (level) {
  case 0:
    break;
...
double ******createA6DMatrix(const int n1, const int n2, const int n3,
    ...
    for (int j = 0; j < n2; j++) {
      ...
      for (int k = 0; k < n3; k++) {
        ...
        for (int l = 0; l < n4; l++) {
          ...
          for (int m = 0; m < n5; m++) {
            mat[i][j][k][l][m] = new double[n6];
            for (int n = 0; n < n6; n++) {
              mat[i][j][k][l][m][n] = init;
            }
          }
        }
      }
...
double *****createA5DMatrix(const int n1, const int n2, const int n3,
                            const int n4, const int n5, const double init) {
  double *****mat = new double ****[n1];
  for (int i = 0; i < n1; i++) {
    mat[i] = new double ***[n2];
    for (int j = 0; j < n2; j++) {
...
double ****createA4DMatrix(const int n1, const int n2, const int n3,
                           const int n4, const double init) {
  double ****mat = new double ***[n1];
  for (int i = 0; i < n1; i++) {
    mat[i] = new double **[n2];
    for (int j = 0; j < n2; j++) {
...
double ***createA3DMatrix(const int n1, const int n2, const int n3,
                          const double init) {
  double ***mat = new double **[n1];
  for (int i = 0; i < n1; i++) {
    mat[i] = new double *[n2];
...
double *createA1DMatrix(const int n1, const double init) {
  double *mat = new double [n1];
  for (int i = 0; i < n1; i++) {
    mat[i] = init;
  }
...
void deleteA1DMatrix(double *mat) {
...
void deleteA6DMatrix(double ******mat, const int n1, const int n2, const int n3,
...
void deleteA5DMatrix(double *****mat, const int n1, const int n2, const int n3,
                     const int n4) {
  for (int i = 0; i < n1; i++) {
    for (int j = 0; j < n2; j++) {
      for (int k = 0; k < n3; k++) {
